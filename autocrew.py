...
def main():
    ...
    if not args.ranking or args.multiple:
        ...
        for i in range(starting_index, starting_index + num_scripts):
            ...
            file_path = save_csv_output(response, overall_goal,
                                        greek_alphabets)
    ...
    if args.ranking:
        ...
        if not csv_file_paths and args.overall_goal:
            ...
            csv_file_paths = [
                f for f in os.listdir(os.getcwd()) if f.endswith('.csv')
                and args.overall_goal.replace(" ", "-") in f and any(
                    greek_alpha in f for greek_alpha in greek_alphabets)
            ]
            ...
            if args.auto_run:
                ...
                for script_file in script_files:
                    if overall_goal_formatted in script_file and top_crew_name in script_file:
                        top_script_path = os.path.join(os.getcwd(),
                                                       script_file)
                        break
